package com.avl.ditest.carsimulator.discovery;
...
public class CarSimDiscovery {
	...
	public List<String> discoverSync() throws IOException {
		List<String> discoveredCarSimulators = new ArrayList<String>();
			...
			while(true) {
				...
				if(receivePacket.getLength() == DISCOVERY_ID.length + DISCOVERY_VERSION.length*2) {
					...
					if(discoveryID.equals(packetID)) {
						...
						discoveredCarSimulators.add("Host: " + receivePacket.getAddress().toString() + " Version: " + versionMajor + "." + versionMinor);
					}
				}
			}
