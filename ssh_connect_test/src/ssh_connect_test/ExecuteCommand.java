...
/**
 * Put your command as an argument arg[0]
 * commands can be:
...
import javax.swing.*;
import java.io.*;
import ssh_connect_test.ClientInfo;
...
public class ExecuteCommand {
	public static void main(String arg) {
		try {
			...
			if (arg != null) {
				...
				command[0] = JOptionPane.showInputDialog("Enter command", "command");
			}

			ConnectionInfo ci = new ConnectionInfo();

			Session session = jsch.getSession(ci.getUsername(), ci.getHostname(), 22);
			...
			((ChannelExec) channel).setCommand(command[1]);
	...
	private static String[] checkCommand(String[] command) {
		if (command[0].equals("kill")) {
			command[1] = "pkill amos-ss17-proj4";
